...
func generateTelemetries() pkg.TelemetryV1Event {
	event := pkg.TelemetryV1Event{
		VersionDiscriminant: pkg.VersionDiscriminant{Version: pkg.V1},
		DeviceId:            fmt.Sprintf("%d", rand.Intn(10000)+5000),
	...
	gens := []metricGen{
		// Device
		{core.Device, "state", "", func() any { return []string{"on", "off", "booting", "maintenance", "error"}[rand.Intn(5)] }},
		{core.Device, "uptime", "s", func() any { return rand.Float64() * 100000 }},
		{core.Device, "battery_level", "%", func() any { return rand.Float64() * 100 }},
		{core.Device, "firmware_version", "", func() any { return "1.2.3" }},
	...
	for i := range count {
		g := gens[indices[i]]
		event.Data = append(event.Data, struct {
