# Personal project: telemetry ingestion workflow in Go
...
![Global architecture](./wiki/attachements/cloud-infrastructure.drawio.svg)
...
When deployed to Azure, each container instance will handle a single event hub partition. 

> [!IMPORTANT]  
> This means that the number of event hub partitions must be equal to the number of container instances deployed. <br>
> Otherwise, some partitions will not be processed, or instances will be idle while waiting for events from their assigned partition.

![Layers pulling](./wiki/attachements/layers-pulling.png)

All the layers share a common implementation pattern: the [Go worker pattern](https://gobyexample.com/worker-pools).
...
> [!NOTE]  
> Each layer has its own configuration as they do not have the same processing needs. <br>

For example, the bronze layer has a lot of I/O operations as it needs to send a lot of metrics messages to the `telemetry-metrics` event hub, so it needs a lot of workers and a large channel buffer. <br>
...
See the [layers configuration](#layers-configuration) section for more details on the configuration of each layer.
