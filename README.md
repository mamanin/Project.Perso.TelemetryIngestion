...
- `metrics_1mo_store`: aggregates metrics into 1 month bins.
...
To provide a friendlier query interface on top of `resolve_metrics`, a set of `metrics_*_bag` functions returns metrics as a property bag `{ metric: value }` grouped by `(timestamp, sensor)`.
